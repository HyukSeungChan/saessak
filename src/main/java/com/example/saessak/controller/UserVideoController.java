...
//            return ResponseEntity.ok(user);
            ...
            return ResponseEntity.status(HttpStatus.OK).body(new ApiResponse("Created","get user video successfully", mapper.writeValueAsString(userVideo)));
        } catch (Exception e) {
    ...
    // 유저-비디오 즐겨찾기 삭제
    @DeleteMapping("/user/video/bookmark/delete")
    public int deleteByUserUserIdAndVideoVideoId(@RequestParam("userId") Long userId, @RequestParam("videoId") int videoId) {
        return userVideoService.deleteByUserUserIdAndVideoVideoId(userId, videoId);
    }
}
